...
#December 7, 2017: Modified by Brooke Stanislawski for Final Project
...
##########################################
experimental_run = TRUE  #running with initial conditions from experimental data?  (includes net radiation)
...
###########################################################################
#Read in dataset from external file# ------------------------------------##CHANGED##------------------------------------
#MATERHORN experimental data: TOWER DATA
# datdir<-"C:/Users/Brooke/Documents/University of Utah/Fall Semester Classes 2017/ATMOS 6240_Land Atmos Interactions/Class Project/Experimental Data/Data/"         #directory where data file resides
# filenm1<-paste(datdir,"PlayaSpring_30minAvg_2013_05_24_GPF_linDetrend.csv",sep="")    #creates string by 'pasting' together contents of objects or other strings
# towerdata<-read.csv(filenm1)
# towerdata<-as.matrix(towerdata)  #converts from "data frame" to "matrix"

#MATERHORN experimental data: RADIATION DATA #using .csv file
filenm2<-paste(datdir,"radiation_data.csv",sep="")    #creates string by 'pasting' together contents of objects or other strings
#csv file above consists of values copied from playa_05_2013_5minDATA.mat
radiationdata<-read.csv(filenm2,header = F)
radiationdata<-as.matrix(radiationdata)  #converts from "data frame" to "matrix"
radiationdata[,4]=radiationdata[,4]-7    #converts from UTC to MST
#rad_data_may_6=radiationdata[1440:1727,]
rad_data_may_6=radiationdata[1524:1811,] #new May 6th data after UTC to MST conversion
rad_data_may_6[205:288,4]=rad_data_may_6[205:288,4]+24  #eliminating negative hr values

index0 = 6422 #May 23, 0700 UTC
index1 = 6698+24 #May 23, 0700 UTC

Rnet_e_hr=aggregate(rad_data_may_6[,10],list(rad_data_may_6[,4]),mean)  #computes the average for every hour for 24 hours
SWdn_e_hr=aggregate(rad_data_may_6[,6],list(rad_data_may_6[,4]),mean) #experimental SWdn hourly average
SWup_e_hr=aggregate(rad_data_may_6[,7],list(rad_data_may_6[,4]),mean) #experimental SWup hourly average
LWdn_e_hr=aggregate(rad_data_may_6[,8],list(rad_data_may_6[,4]),mean) #experimental SWdn hourly average

Rnet_e = rep(0,25)                 #initialize hourly Rnet vector with zeros
SWdn_e = rep(0,25)                         #initialize hourly SWdn vector with zeros
SWup_e = rep(0,25)
LWdn_e = rep(LWdn_e_hr[1,2],25)            #initialize hourly LWdn vector with first LWdn value
for (i in 2:length(Rnet_e)) {      #loop to populate Rnet vector with hourly averages
  Rnet_e[i]=Rnet_e_hr[i-1,2]
  SWdn_e[i]=SWdn_e_hr[i-1,2]
  SWup_e[i]=SWup_e_hr[i-1,2]
  LWdn_e[i]=LWdn_e_hr[i-1,2]
}
names(Rnet_e)<-t.hr
...
#comment out if want to force Rnet with experimental data
if (experimental_run == TRUE) {  ##CHANGED##
  SWdn = SWdn_e
  names(SWdn)<-t.hr
}
...
albedo.c<-mean(SWup_e)/mean(SWdn_e)    #surface albedo (Hang et al. [2016]) ##CHANGED##
#albedo.c<-0.3    #surface albedo (Hang et al. [2016]) ##CHANGED##
albedo<-albedo.c #surface albedo 
z0<-0.00046     #roughness length [m] for desert, accourding to Stull [1988] ##CHANGED##
epsilon.s<-0.93  #surface emissivity for bare soil, according to Jin & Liang [2006] ##CHANGED##
dt<-60           #model timestep [s]
t.day<-5     	 #Run time in days ##CHANGED##
tmax<-t.day*24*3600  #maximum time [s]

#Compute u_star ##CHANGED##
...
#D<-0.1
#CHANGE D to .7        #measured by soil probes
D<-0.7 #0.1*(1/24)     #Morrison et al [2017]     #the depth of soil that temp fluctuations would penetrate [m]; 0.1m is roughly the depth that would penetrate on diurnal timescales
Cp.soil<-1542.8        ##CHANGED## calculated based on rho for May 22  #specific heat of soil organic material [J/kg/K]
rho.soil<-1425         ##CHANGED## #density of soil organic material [kg/m3]
...
zr<-25          #reference height [m] where Ur applies: top tower measurement location (where L is computed)
##########################################
...
ra.f<-function(Ur=1,zr=50,z0=z0,d=0,rho=1){
  ...
  CD<-(k^2)/(log((zr-d)/z0))^2   #aerodynamic transfer coefficient according to MOST in Garrett [1992] "The atmospheric boundary layer"
...
f<-function(T,Ta,SWdn,LWdn,albedo,epsilon.s,Ur,zr,z0,gvmax=gvmax){
  ...
  #comment out if want to force LWdn and SWdn with experimental data and let code calculate Rnet
  # if (experimental_run == TRUE) {
  #   Rnet = Rnet_e
  #   names(Rnet)<-t.hr
  # }
...
while(tcurr<tmax){
  ...
  #comment out if want to force Rnet with experimental data
  # if (experimental_run == TRUE) {   ##CHANGED##
  #   Rnet = approx(x=as.numeric(names(Rnet_e))*3600,y=Rnet_e,xout=tcurr%%(24*3600))$y
  # }
